---
title: "goal17_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 17.
...
This is all the indicator data for Sustainable Development Goal 17 for SIDS.
...
```{r}
unique(goal17$SeriesDescription)
```
...
```{r}
goal17_tidy_grouped_most_recent <- goal17_tidy_grouped %>% 
...
```{r}
goal17_tidy_grouped_most_recent_no_time <- goal17_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal17_wider <- goal17_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal17_wider, here("./most_recent/data_cleaned_and_summarized/goal17_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal17_tidy_grouped_most_recent['TimePeriod'])
