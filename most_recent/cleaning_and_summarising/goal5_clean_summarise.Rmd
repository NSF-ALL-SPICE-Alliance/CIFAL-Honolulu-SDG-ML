---
title: "goal5_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 5.
...
This is all the indicator data for Sustainable Development Goal 5 for SIDS.
...
```{r}
unique(goal5$SeriesDescription)
```
...
```{r}
goal5_tidy_grouped_most_recent <- goal5_tidy_grouped %>% 
...
```{r}
goal5_tidy_grouped_most_recent_no_time <- goal5_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal5_wider <- goal5_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal5_wider, here("./most_recent/data_cleaned_and_summarized/goal5_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal5_tidy_grouped_most_recent['TimePeriod'])
