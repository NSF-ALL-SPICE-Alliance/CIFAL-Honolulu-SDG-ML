---
title: "goal6_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 6.
...
This is all the indicator data for Sustainable Development Goal 6 for SIDS.
...
```{r}
unique(goal6$SeriesDescription)
```
...
```{r}
goal6_tidy_grouped_most_recent <- goal6_tidy_grouped %>% 
...
```{r}
goal6_tidy_grouped_most_recent_no_time <- goal6_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal6_wider <- goal6_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal6_wider, here("./most_recent/data_cleaned_and_summarized/goal6_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal6_tidy_grouped_most_recent['TimePeriod'])
