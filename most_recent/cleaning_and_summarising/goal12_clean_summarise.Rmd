---
title: "goal12_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 12.
...
This is all the indicator data for Sustainable Development Goal 12 for SIDS.
...
```{r}
unique(goal12$SeriesDescription)
```
...
```{r}
goal12_tidy_grouped_most_recent <- goal12_tidy_grouped %>% 
...
```{r}
goal12_tidy_grouped_most_recent_no_time <- goal12_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal12_wider <- goal12_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal12_wider, here("./most_recent/data_cleaned_and_summarized/goal12_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal12_tidy_grouped_most_recent['TimePeriod'])
