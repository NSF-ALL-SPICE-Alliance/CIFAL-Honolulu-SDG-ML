---
title: "goal7_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 7.
...
This is all the indicator data for Sustainable Development Goal 7 for SIDS.
...
```{r}
unique(goal7$SeriesDescription)
```
...
```{r}
goal7_tidy_grouped_most_recent <- goal7_tidy_grouped %>% 
...
```{r}
goal7_tidy_grouped_most_recent_no_time <- goal7_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal7_wider <- goal7_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal7_wider, here("./most_recent/data_cleaned_and_summarized/goal7_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal7_tidy_grouped_most_recent['TimePeriod'])
