---
title: "goal11_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 11.
...
This is all the indicator data for Sustainable Development Goal 11 for SIDS.
...
```{r}
unique(goal11$SeriesDescription)
```
...
```{r}
goal11_tidy_grouped_most_recent <- goal11_tidy_grouped %>% 
...
```{r}
goal11_tidy_grouped_most_recent_no_time <- goal11_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal11_wider <- goal11_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal11_wider, here("./most_recent/data_cleaned_and_summarized/goal11_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal11_tidy_grouped_most_recent['TimePeriod'])
