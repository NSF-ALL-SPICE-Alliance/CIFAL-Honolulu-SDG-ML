---
title: "goal9_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 9.
...
This is all the indicator data for Sustainable Development Goal 9 for SIDS.
...
```{r}
unique(goal9$SeriesDescription)
```
...
```{r}
goal9_tidy_grouped_most_recent <- goal9_tidy_grouped %>% 
...
```{r}
goal9_tidy_grouped_most_recent_no_time <- goal9_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal9_wider <- goal9_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal9_wider, here("./most_recent/data_cleaned_and_summarized/goal9_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal9_tidy_grouped_most_recent['TimePeriod'])
