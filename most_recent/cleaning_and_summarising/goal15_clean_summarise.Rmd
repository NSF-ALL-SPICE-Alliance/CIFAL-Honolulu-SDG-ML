---
title: "goal15_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 15.
...
This is all the indicator data for Sustainable Development Goal 15 for SIDS.
...
```{r}
unique(goal15$SeriesDescription)
```
...
```{r}
goal15_tidy_grouped_most_recent <- goal15_tidy_grouped %>% 
...
```{r}
goal15_tidy_grouped_most_recent_no_time <- goal15_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal15_wider <- goal15_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal15_wider, here("./most_recent/data_cleaned_and_summarized/goal15_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal15_tidy_grouped_most_recent['TimePeriod'])
