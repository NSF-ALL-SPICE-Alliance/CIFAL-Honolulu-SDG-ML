---
title: "goal10_clean_summarise"
author: "Connor Flynn"
date: "7/22/2022"
output: html_document
---

Goal: The goal of this Rmd is to clean and summarize the data on Sustainable Development Goal 10.
...
This is all the indicator data for Sustainable Development Goal 10 for SIDS.
...
```{r}
unique(goal10$SeriesDescription)
```
...
```{r}
goal10_tidy_grouped_most_recent <- goal10_tidy_grouped %>% 
...
```{r}
goal10_tidy_grouped_most_recent_no_time <- goal10_tidy_grouped_most_recent %>% 
  select(!TimePeriod)
```

Pivot data to get one SIDS per row and one indicator per column
```{r}
goal10_wider <- goal10_tidy_grouped_most_recent_no_time %>% 
...
```{r}
write_csv(goal10_wider, here("./most_recent/data_cleaned_and_summarized/goal10_most_recent.csv"))
```
...
```{r}
most_recent_data <- table(goal10_tidy_grouped_most_recent['TimePeriod'])
